# Hikyuu × Qlib 量化交易平台 - 开发总纲

**项目名称**: Hikyuu × Qlib - AI 驱动的量化交易平台
**架构模式**: Hexagonal Architecture (六边形架构) + Domain-Driven Design (DDD)
**开发方法**: Test-Driven Development (TDD)
**版本**: 2.0.0
**最后更新**: 2025-11-11

---

## 📋 目录

1. [项目概述](#项目概述)
2. [架构原则](#架构原则)
3. [开发约束](#开发约束)
4. [TDD 强制要求](#tdd-强制要求)
5. [代码质量标准](#代码质量标准)
6. [分层开发规范](#分层开发规范)
7. [测试策略](#测试策略)
8. [Git 工作流](#git-工作流)
9. [文档要求](#文档要求)
10. [性能与安全](#性能与安全)

---

## 项目概述

### 项目目标

整合 **Hikyuu** (中国量化交易框架) 和 **Qlib** (微软 AI 量化投资平台),打造一个:
- ✅ **高度可测试**的量化交易系统
- ✅ **框架无关**的核心业务逻辑
- ✅ **易于扩展**的模块化架构
- ✅ **生产级**的代码质量标准

### 核心功能模块

| 模块 | 功能 | 状态 |
|------|------|------|
| **数据处理** | Hikyuu 数据加载、Qlib 格式转换、数据验证 | 待开发 |
| **模型训练** | Qlib 模型训练、超参优化、实验跟踪 | 待开发 |
| **策略执行** | Hikyuu 回测、信号转换、组合优化 | 待开发 |
| **分析报告** | 绩效分析、可视化、策略对比 | 待开发 |
| **CLI 工具** | 命令行界面、配置管理、任务调度 | 待开发 |

### 开发环境

**Anaconda 环境**: `qlib_hikyuu`

- 所有开发工作必须在此 conda 环境中进行
- 激活命令: `conda activate qlib_hikyuu`
- Python 版本: 3.8+

### 技术栈

```yaml
语言: Python 3.8+
开发环境: Anaconda (qlib_hikyuu)
测试框架: Pytest
依赖注入: dependency-injector
配置管理: Pydantic
日志管理: Loguru
异步编程: asyncio
数据库: PostgreSQL (asyncpg)
量化框架:
  - Hikyuu (数据、指标、回测)
  - Qlib (模型、预测、实验)
代码质量:
  - 类型检查: mypy
  - 代码检查: ruff
  - 格式化: black / ruff format
```

---

## 架构原则

### 核心原则 (不可违反)

#### 1. 依赖规则 (Dependency Rule)

**强制要求**: 依赖只能向内,内层对外层一无所知

```
┌─────────────────────────────────────┐
│   Adapters (Hikyuu/Qlib/CLI)       │  ← 外层 (可替换)
│   - 实现 Ports                      │
│   - 调用外部框架                    │
└──────────────┬──────────────────────┘
               │ 依赖 (向内)
┌──────────────▼──────────────────────┐
│   Use Cases (业务流程编排)          │  ← 中层 (应用)
│   - 依赖 Ports (接口)               │
│   - 编排 Domain 对象                │
└──────────────┬──────────────────────┘
               │ 依赖 (向内)
┌──────────────▼──────────────────────┐
│   Domain (核心业务规则)             │  ← 内层 (核心)
│   - 零外部依赖                      │
│   - 定义 Ports (接口)               │
│   - 纯业务逻辑                      │
└─────────────────────────────────────┘

✅ 允许: Adapters → Use Cases → Domain
❌ 禁止: Domain → Use Cases
❌ 禁止: Domain → Adapters
❌ 禁止: Domain → 任何框架 (Hikyuu/Qlib/FastAPI/等)
```

**检查方式**: 运行架构测试 `pytest tests/architecture/test_dependency_rules.py`

#### 2. 框架独立性 (Framework Independence)

**强制要求**: Domain 层绝对不能 import 任何外部框架

```python
# ✅ 正确: Domain 层代码
# domain/entities/stock.py
from dataclasses import dataclass
from domain.value_objects.stock_code import StockCode

@dataclass
class Stock:
    code: StockCode
    name: str

    def is_tradable(self) -> bool:
        return self.is_active

# ❌ 错误: Domain 层不能这样写
import hikyuu as hku  # ❌ 禁止 import 框架
import qlib  # ❌ 禁止 import 框架
from fastapi import APIRouter  # ❌ 禁止 import 框架
```

**原因**:
- Domain 是业务的核心,必须独立于技术细节
- 框架可以替换,业务规则不变
- 便于测试,无需启动框架

#### 3. 依赖倒置原则 (Dependency Inversion)

**强制要求**: Domain 定义接口 (Ports), Adapters 实现接口

```python
# ✅ 正确模式

# Step 1: Domain 定义接口 (Port)
# domain/ports/stock_data_provider.py
from abc import ABC, abstractmethod

class IStockDataProvider(ABC):
    """Port: Domain 定义接口"""
    @abstractmethod
    async def load_stock_data(self, code: StockCode) -> pd.DataFrame:
        pass

# Step 2: Use Case 依赖接口
# use_cases/data/load_stock_data.py
class LoadStockDataUseCase:
    def __init__(self, provider: IStockDataProvider):  # 依赖抽象
        self.provider = provider

    async def execute(self, request):
        data = await self.provider.load_stock_data(...)  # 使用接口
        return response

# Step 3: Adapter 实现接口
# adapters/hikyuu/data_adapter.py
class HikyuuDataAdapter(IStockDataProvider):
    """实现 Domain Port"""
    async def load_stock_data(self, code: StockCode):
        # 调用 Hikyuu API
        stock = hku.Stock(code.value)
        return stock.getKData(...)

# ❌ 错误模式: 直接依赖具体实现
class LoadStockDataUseCase:
    def __init__(self):
        self.hikyuu = HikyuuDataLoader()  # ❌ 不能直接依赖具体类
```

#### 4. 单一职责原则 (Single Responsibility)

**每层只做一件事**:

| 层次 | 职责 | 禁止事项 |
|------|------|---------|
| **Domain** | 定义业务规则和模型 | ❌ 调用数据库<br>❌ 调用 API<br>❌ 处理 HTTP |
| **Use Cases** | 编排业务流程 | ❌ 包含业务规则<br>❌ 直接调用框架 |
| **Adapters** | 实现技术细节 | ❌ 包含业务逻辑 |
| **Infrastructure** | 提供技术支持 | ❌ 包含业务逻辑 |

---

## 开发约束

### 🚫 绝对禁止事项

#### 1. Domain 层禁止事项

```python
# ❌ 禁止 #1: import 外部框架
import hikyuu  # ❌
import qlib  # ❌
from sqlalchemy import Column  # ❌
from fastapi import HTTPException  # ❌

# ❌ 禁止 #2: 依赖具体实现
class Stock:
    def __init__(self):
        self.db = PostgresDB()  # ❌ 不能依赖具体数据库

# ❌ 禁止 #3: 包含技术细节
class Order:
    def save_to_database(self):  # ❌ 不能有数据库逻辑
        conn = psycopg2.connect(...)

# ✅ 正确: 纯业务逻辑
@dataclass
class Stock:
    code: StockCode
    name: str

    def is_tradable(self) -> bool:
        """纯业务规则"""
        return self.is_active and self.market.is_open()
```

#### 2. Use Cases 禁止事项

```python
# ❌ 禁止: 直接调用框架
class TrainModelUseCase:
    async def execute(self, request):
        qlib.init(...)  # ❌ 不能直接调用 Qlib
        model = LGBModel(...)  # ❌ 不能直接创建框架对象

# ✅ 正确: 通过 Port 接口
class TrainModelUseCase:
    def __init__(self, trainer: IModelTrainer):  # Port
        self.trainer = trainer

    async def execute(self, request):
        model = await self.trainer.train(...)  # 使用接口
```

#### 3. 测试禁止事项

```python
# ❌ 禁止: 先写代码,后写测试
def create_user(email, name):  # ❌ 没有先写测试
    ...

# ✅ 正确: TDD 流程
# Step 1: 先写测试
def test_create_user_with_valid_email():
    use_case = CreateUserUseCase(mock_repo)
    result = use_case.execute(...)
    assert result.success is True

# Step 2: 运行测试 (失败)
# Step 3: 写最小代码让测试通过
# Step 4: 重构
```

### ✅ 强制要求

#### 1. 所有 Public 方法必须有类型注解

```python
# ❌ 错误: 缺少类型注解
def load_data(code, start, end):
    pass

# ✅ 正确: 完整类型注解
async def load_data(
    code: StockCode,
    start: datetime,
    end: datetime
) -> pd.DataFrame:
    pass
```

#### 2. 所有类/方法必须有文档字符串

```python
# ❌ 错误: 缺少文档
class Stock:
    def is_tradable(self):
        return True

# ✅ 正确: 完整文档
class Stock:
    """股票实体

    表示一个可交易的股票,包含代码、名称等属性。

    Attributes:
        code: 股票代码 (StockCode 值对象)
        name: 股票名称
        is_active: 是否活跃

    Examples:
        >>> stock = Stock(StockCode("sh000001"), "上证指数", True)
        >>> stock.is_tradable()
        True
    """

    def is_tradable(self) -> bool:
        """判断股票是否可交易

        业务规则: 只有活跃的股票才能交易

        Returns:
            bool: 可交易返回 True,否则 False
        """
        return self.is_active
```

#### 3. 每个模块必须有 README.md

```
src/domain/
├── README.md          ✅ 必须
├── .claude.md         ✅ 必须
├── entities/
│   └── README.md      ✅ 必须
```

---

## TDD 强制要求

### 🔴 RED → 🟢 GREEN → 🔵 REFACTOR 循环

**这是铁律,必须严格遵守**:

#### Step 1: 🔴 RED (编写失败的测试)

```python
# tests/unit/domain/entities/test_stock.py

def test_stock_is_tradable_when_active():
    """🔴 RED: 先写测试"""
    # Given: 活跃的股票
    stock = Stock(
        code=StockCode("sh000001"),
        name="上证指数",
        is_active=True
    )

    # When: 判断是否可交易
    result = stock.is_tradable()

    # Then: 应该可交易
    assert result is True

def test_stock_not_tradable_when_inactive():
    """🔴 RED: 测试边界情况"""
    stock = Stock(
        code=StockCode("sh000001"),
        name="上证指数",
        is_active=False
    )

    assert stock.is_tradable() is False
```

**运行测试**:
```bash
pytest tests/unit/domain/entities/test_stock.py -v
# 预期: FAILED (因为还没实现)
```

#### Step 2: 🟢 GREEN (最小实现)

```python
# src/domain/entities/stock.py

@dataclass
class Stock:
    """最小实现,只为通过测试"""
    code: StockCode
    name: str
    is_active: bool = True

    def is_tradable(self) -> bool:
        """最简单的实现"""
        return self.is_active  # 只写这一行
```

**运行测试**:
```bash
pytest tests/unit/domain/entities/test_stock.py -v
# 预期: PASSED ✅
```

#### Step 3: 🔵 REFACTOR (重构优化)

```python
# 重构: 添加更复杂的业务规则 (如果测试要求)
def is_tradable(self) -> bool:
    """重构后的实现"""
    return self.is_active and self.market.is_open()
```

**运行测试**:
```bash
pytest tests/unit/domain/entities/test_stock.py -v
# 预期: 仍然 PASSED ✅
```

### 测试覆盖率强制要求

**每层都有最低覆盖率要求,不达标不允许合并**:

| 层次 | 最低覆盖率 | 推荐覆盖率 | 检查命令 |
|------|-----------|-----------|---------|
| **Domain** | 95% | 100% | `pytest --cov=src/domain --cov-report=term-missing` |
| **Use Cases** | 90% | 95% | `pytest --cov=src/use_cases --cov-report=term-missing` |
| **Adapters** | 85% | 90% | `pytest --cov=src/adapters --cov-report=term-missing` |
| **Infrastructure** | 88% | 90% | `pytest --cov=src/infrastructure --cov-report=term-missing` |
| **整体** | 90% | 95% | `pytest --cov=src --cov-report=html` |

**CI/CD 检查**:
```bash
# 在 CI 中运行,覆盖率不达标则构建失败
pytest --cov=src --cov-fail-under=90
```

### 测试命名规范

```python
# ✅ 正确: 描述性测试名称
def test_create_user_with_valid_email_returns_success():
    pass

def test_create_user_with_duplicate_email_returns_error():
    pass

def test_stock_is_tradable_when_market_is_open():
    pass

# ❌ 错误: 模糊的测试名称
def test_user():
    pass

def test_1():
    pass
```

### 测试组织结构

```
tests/
├── unit/                    # 单元测试
│   ├── domain/
│   │   ├── entities/
│   │   │   ├── test_stock.py
│   │   │   └── test_model.py
│   │   ├── value_objects/
│   │   │   ├── test_stock_code.py
│   │   │   └── test_price.py
│   │   └── services/
│   ├── use_cases/
│   │   ├── data/
│   │   └── models/
│   └── adapters/
├── integration/             # 集成测试
│   ├── test_hikyuu_integration.py
│   ├── test_qlib_integration.py
│   └── test_end_to_end.py
├── architecture/            # 架构测试
│   ├── test_dependency_rules.py
│   └── test_layer_isolation.py
└── conftest.py             # Pytest 配置
```

---

## 代码质量标准

### 强制性代码检查

**所有代码必须通过以下检查才能提交**:

#### 1. Type Checking (mypy)

```bash
# 运行类型检查
mypy src/ --strict

# 配置文件: pyproject.toml
[tool.mypy]
python_version = "3.8"
strict = true
warn_return_any = true
warn_unused_configs = true
disallow_untyped_defs = true
```

**示例**:
```python
# ✅ 通过类型检查
def load_data(code: StockCode) -> pd.DataFrame:
    pass

# ❌ 不通过类型检查
def load_data(code):  # Missing type annotations
    pass
```

#### 2. Linting (ruff)

```bash
# 运行代码检查
ruff check src/

# 配置文件: pyproject.toml
[tool.ruff]
line-length = 100
select = ["E", "F", "I", "N", "W"]
ignore = ["E501"]
```

#### 3. Formatting (ruff format / black)

```bash
# 格式化代码
ruff format src/
# 或
black src/

# 检查格式
ruff format --check src/
```

#### 4. Import Sorting (ruff / isort)

```python
# ✅ 正确的 import 顺序
# 1. 标准库
import os
from datetime import datetime
from typing import Optional

# 2. 第三方库
import pandas as pd
import pytest

# 3. 本地模块
from domain.entities.stock import Stock
from domain.value_objects.stock_code import StockCode
```

### 代码复杂度限制

```python
# 配置文件: .flake8 或 pyproject.toml
[flake8]
max-complexity = 10          # 圈复杂度 <= 10
max-line-length = 100        # 单行 <= 100 字符
max-function-length = 50     # 函数 <= 50 行
```

### Code Review 检查清单

**每个 PR 必须检查**:

- [ ] 所有测试通过 (`pytest`)
- [ ] 覆盖率达标 (`pytest --cov=src --cov-fail-under=90`)
- [ ] 类型检查通过 (`mypy src/ --strict`)
- [ ] 代码检查通过 (`ruff check src/`)
- [ ] 格式检查通过 (`ruff format --check src/`)
- [ ] 架构测试通过 (`pytest tests/architecture/`)
- [ ] 文档完整 (所有 public 方法有 docstring)
- [ ] 遵循依赖规则 (内层不依赖外层)
- [ ] 无安全漏洞 (`bandit -r src/`)
- [ ] Commit 消息规范 (Conventional Commits)

---

## 分层开发规范

### Domain 层开发规范

**参考**: [src/domain/.claude.md](src/domain/.claude.md)

**核心约束**:
1. ✅ 零外部依赖 (无 `import hikyuu`, `import qlib`)
2. ✅ 纯 Python 标准库 + typing
3. ✅ 所有 Ports 定义在此层
4. ✅ 覆盖率 >= 95%

**开发流程**:
```
1. 从 Value Objects 开始 (最简单,无依赖)
2. 然后 Entities (依赖 Value Objects)
3. 然后 Aggregates (依赖 Entities)
4. 定义 Ports (接口)
5. 实现 Domain Services
6. 定义 Domain Events
```

**示例**:
```python
# ✅ 正确的 Domain 代码
@dataclass(frozen=True)
class StockCode:
    """Value Object: 不可变,无副作用"""
    value: str

    def __post_init__(self):
        if not self._is_valid():
            raise ValueError(f"Invalid stock code: {self.value}")

    def _is_valid(self) -> bool:
        """业务规则: 股票代码格式验证"""
        return len(self.value) == 8 and self.value[:2] in ["sh", "sz"]
```

### Use Cases 层开发规范

**参考**: [src/use_cases/.claude.md](src/use_cases/.claude.md)

**核心约束**:
1. ✅ 只依赖 Domain (通过 Ports)
2. ✅ 使用 Request/Response DTOs
3. ✅ 编排业务流程,不包含业务规则
4. ✅ 覆盖率 >= 90%

**开发流程**:
```
1. 定义 Request DTO
2. 定义 Response DTO
3. 编写测试 (Mock Ports)
4. 实现 Use Case (编排 Domain 对象)
```

**示例**:
```python
# ✅ 正确的 Use Case 代码
@dataclass
class TrainModelRequest:
    """Request DTO"""
    model_type: str
    dataset_config: Dict[str, Any]

@dataclass
class TrainModelResponse:
    """Response DTO"""
    model: Optional[Model]
    success: bool
    error: Optional[str] = None

class TrainModelUseCase:
    def __init__(
        self,
        data_provider: IDataProvider,  # Port 依赖
        trainer: IModelTrainer         # Port 依赖
    ):
        self.data_provider = data_provider
        self.trainer = trainer

    async def execute(self, request: TrainModelRequest) -> TrainModelResponse:
        # 1. 加载数据 (通过 Port)
        dataset = await self.data_provider.load_dataset(...)

        # 2. 验证 (调用 Domain)
        if not dataset.is_valid():
            return TrainModelResponse(success=False, error="Invalid dataset")

        # 3. 训练 (通过 Port)
        model = await self.trainer.train(...)

        # 4. 应用业务规则 (调用 Domain)
        model.mark_as_trained()

        return TrainModelResponse(model=model, success=True)
```

### Adapters 层开发规范

**参考**: [src/adapters/.claude.md](src/adapters/.claude.md)

**核心约束**:
1. ✅ 实现 Domain Ports
2. ✅ 可以调用外部框架 (Hikyuu, Qlib)
3. ✅ 不包含业务逻辑
4. ✅ 覆盖率 >= 85%

**开发流程**:
```
1. 找到要实现的 Port 接口
2. 编写测试 (Mock 框架)
3. 实现 Adapter
4. 进行转换 (框架对象 ↔ Domain 对象)
```

**示例**:
```python
# ✅ 正确的 Adapter 代码
class HikyuuDataAdapter(IStockDataProvider):
    """实现 Domain Port"""

    def __init__(self, config_file: str):
        self.config_file = config_file
        self._initialized = False

    async def load_stock_data(
        self,
        code: StockCode,      # Domain 对象输入
        date_range: DateRange  # Domain 对象输入
    ) -> pd.DataFrame:
        self._ensure_initialized()

        # 1. Domain → Hikyuu 转换
        stock = hku.Stock(code.value)
        query = hku.Query(
            start=hku.Datetime(date_range.start),
            end=hku.Datetime(date_range.end)
        )

        # 2. 调用 Hikyuu API
        kdata = stock.getKData(query)

        # 3. Hikyuu → DataFrame 转换
        return self._to_dataframe(kdata)
```

### Infrastructure 层开发规范

**参考**: [src/infrastructure/.claude.md](src/infrastructure/.claude.md)

**核心约束**:
1. ✅ 提供技术支持 (配置、日志、DI、数据库)
2. ✅ 不包含业务逻辑
3. ✅ 覆盖率 >= 88%

---

## 测试策略

### 测试金字塔

```
        ┌──────────┐
        │ E2E (10%)│  集成测试 (少量,覆盖关键路径)
        └──────────┘
      ┌──────────────┐
      │Integration   │  集成测试 (适量,测试层间交互)
      │   (20%)      │
      └──────────────┘
    ┌──────────────────┐
    │   Unit Tests     │  单元测试 (大量,快速,隔离)
    │     (70%)        │
    └──────────────────┘
```

### 各层测试策略

#### Domain 层测试

**特点**: 纯单元测试,无 Mock

```python
# ✅ Domain 测试: 无 Mock
def test_stock_code_validation():
    """纯业务逻辑测试"""
    # Valid
    code = StockCode("sh000001")
    assert code.value == "sh000001"

    # Invalid
    with pytest.raises(ValueError):
        StockCode("invalid")
```

#### Use Case 层测试

**特点**: Mock Ports

```python
# ✅ Use Case 测试: Mock Ports
@pytest.fixture
def mock_data_provider():
    provider = Mock(spec=IDataProvider)
    provider.load_dataset.return_value = Mock()
    return provider

async def test_train_model_use_case(mock_data_provider):
    """测试业务流程编排"""
    use_case = TrainModelUseCase(
        data_provider=mock_data_provider,
        trainer=Mock(spec=IModelTrainer)
    )

    response = await use_case.execute(request)

    assert response.success is True
    mock_data_provider.load_dataset.assert_called_once()
```

#### Adapter 层测试

**特点**: Mock 外部框架

```python
# ✅ Adapter 测试: Mock 框架
@patch('hikyuu.Stock')
def test_hikyuu_data_adapter(mock_stock):
    """测试框架调用"""
    mock_stock.return_value.getKData.return_value = [...]

    adapter = HikyuuDataAdapter()
    data = await adapter.load_stock_data(...)

    assert isinstance(data, pd.DataFrame)
    mock_stock.assert_called_once()
```

### 架构测试

**测试依赖规则**:

```python
# tests/architecture/test_dependency_rules.py
import pytest
from pathlib import Path

def test_domain_has_no_external_dependencies():
    """Domain 层不能 import 外部框架"""
    domain_files = Path("src/domain").rglob("*.py")

    forbidden_imports = ["hikyuu", "qlib", "sqlalchemy", "fastapi"]

    for file in domain_files:
        content = file.read_text()
        for forbidden in forbidden_imports:
            assert forbidden not in content, \
                f"{file} imports forbidden module: {forbidden}"

def test_use_cases_only_depend_on_domain():
    """Use Cases 只依赖 Domain"""
    # 实现检查逻辑
    pass
```

---

## Git 工作流

### 分支策略

```
main              (生产环境,受保护)
  ├── develop     (开发环境)
  │   ├── feature/domain-entities
  │   ├── feature/use-cases-data
  │   └── feature/adapters-hikyuu
  └── hotfix/critical-bug
```

### Commit 消息规范 (Conventional Commits)

```bash
<type>(<scope>): <subject>

<body>

<footer>
```

**类型 (type)**:
- `feat`: 新功能
- `fix`: Bug 修复
- `refactor`: 重构 (不改变功能)
- `test`: 添加/修改测试
- `docs`: 文档更新
- `style`: 代码格式 (不影响逻辑)
- `chore`: 构建/工具变更

**示例**:
```bash
feat(domain): add Stock entity with tradability rules

- Implement Stock entity with StockCode value object
- Add is_tradable() business rule
- Add comprehensive unit tests (100% coverage)

Closes #42
```

### PR 要求

**每个 PR 必须**:
1. 标题遵循 Conventional Commits
2. 包含完整测试 (覆盖率达标)
3. 通过所有 CI 检查
4. 至少 1 个 Approve
5. 无 merge conflicts
6. 更新相关文档

**PR 模板**:
```markdown
## 变更类型
- [ ] feat (新功能)
- [ ] fix (Bug 修复)
- [ ] refactor (重构)
- [ ] test (测试)
- [ ] docs (文档)

## 变更描述
简要描述这个 PR 的变更内容

## 测试覆盖率
- Domain: XX%
- Use Cases: XX%
- Adapters: XX%

## 检查清单
- [ ] 所有测试通过
- [ ] 覆盖率达标
- [ ] 类型检查通过
- [ ] 代码检查通过
- [ ] 文档已更新
- [ ] 遵循架构原则
```

---

## 文档要求

### 必须的文档

#### 1. 每层的 README.md

```markdown
# Domain 层

## 概述
...

## 目录结构
...

## 开发指南
...

## 示例
...
```

#### 2. 每个模块的 .claude.md

包含:
- 模块职责
- TDD 开发流程
- 测试覆盖率目标
- 代码示例
- 交付清单

#### 3. API 文档

```python
# 使用 docstring 自动生成
def load_stock_data(code: StockCode) -> pd.DataFrame:
    """加载股票数据

    Args:
        code: 股票代码

    Returns:
        股票数据 DataFrame

    Raises:
        ValueError: 代码无效时

    Examples:
        >>> data = load_stock_data(StockCode("sh000001"))
        >>> data.shape
        (1000, 7)
    """
```

---

## 性能与安全


### 安全要求

1. **不提交敏感信息**
   ```bash
   # .gitignore
   *.env
   config/secrets.yaml
   credentials.json
   ```

2. **依赖扫描**
   ```bash
   # 检查已知漏洞
   safety check
   pip-audit
   ```

3. **代码扫描**
   ```bash
   # 检查安全问题
   bandit -r src/
   ```

---

## 快速参考

### 常用命令

```bash
# 运行所有测试
pytest

# 运行特定层测试
pytest tests/unit/domain/ -v

# 测试覆盖率
pytest --cov=src --cov-report=html

# 类型检查
mypy src/ --strict

# 代码检查
ruff check src/

# 格式化
ruff format src/

# 架构测试
pytest tests/architecture/ -v

# 安全扫描
bandit -r src/
```

### 开发流程总结

```
1. 🔴 RED: 编写失败测试
   └─> pytest (预期 FAILED)

2. 🟢 GREEN: 最小实现
   └─> pytest (预期 PASSED)

3. 🔵 REFACTOR: 重构优化
   └─> pytest (预期仍 PASSED)

4. 质量检查
   ├─> mypy src/ --strict
   ├─> ruff check src/
   ├─> pytest --cov=src --cov-fail-under=90
   └─> pytest tests/architecture/

5. 提交代码
   └─> git commit -m "feat(domain): add Stock entity"

6. 更新项目状态文档src/PROJECT_STATUS.md中对应任务的进度

6. 创建 PR
   └─> 等待 Review & CI 通过

7. 合并到 develop
```

---

## 相关文档

### 架构文档
- [src/ARCHITECTURE.md](src/ARCHITECTURE.md) - 完整架构文档
- [src/README.md](src/README.md) - 快速开始
- [src/MIGRATION_GUIDE.md](src/MIGRATION_GUIDE.md) - 迁移指南
- [src/PROJECT_STATUS.md](src/PROJECT_STATUS.md) - 项目状态

### 各层开发指南
- [src/domain/.claude.md](src/domain/.claude.md) - Domain 层
- [src/use_cases/.claude.md](src/use_cases/.claude.md) - Use Cases 层
- [src/adapters/.claude.md](src/adapters/.claude.md) - Adapters 层
- [src/infrastructure/.claude.md](src/infrastructure/.claude.md) - Infrastructure 层

### 外部参考
- [Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture/)
- [Domain-Driven Design](https://martinfowler.com/tags/domain%20driven%20design.html)
- [Clean Architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html)

---

**项目负责人**: Architecture Team
**架构师**: Claude AI
**版本**: 2.0.0
**最后更新**: 2025-11-11

**核心理念**:
> "让业务逻辑独立于技术细节,让测试驱动设计,让架构保持清晰"

---

## ⚠️ 重要提醒

1. **Domain 层绝对不能依赖外部框架** - 这是不可违反的铁律
2. **必须遵循 TDD** - 先写测试,后写代码
3. **测试覆盖率必须达标** - Domain ≥95%, Use Cases ≥90%, Adapters ≥85%
4. **依赖只能向内** - 外层可以依赖内层,反之禁止
5. **所有 Public 方法必须有类型注解和文档字符串**
6. **代码提交前必须通过所有质量检查** (mypy, ruff, pytest, coverage)

## 研发红线
**开发工作准则：**

1. **以暗猜接口为耻，以认真查阅为荣**：所有接口设计必须基于现有文档，禁止猜测接口定义
2. **以模糊执行为耻，以寻求确认为荣**：所有执行步骤必须明确，模糊需求需主动确认
3. **以盲想业务为耻，以人类确认为荣**：业务需求必须通过用户确认，禁止主观假设
4. **以创造接口为耻，以复用现有为荣**：优先复用现有接口，避免创造不必要的接口
5. **以跳过验证为耻，以主动测试为荣**：所有功能必须经过验证，禁止跳过测试
6. **以破坏架构为耻，以遵循规范为荣**：遵循现有架构规范，禁止破坏系统架构
7. **以假装理解为耻，以诚实无知为荣**：遇到不理解的问题要诚实承认，禁止假装理解，要向deepwiki-mcp询问qlib和hikyuu框架的使用问题。
8. **以盲目修改为耻，以谨慎重构为荣**：修改代码要谨慎，禁止盲目修改现有功能
9. **以英文对话为耻，以中文对话为荣**：和主人对话要用中文
**违反上述任何一条,PR 将被拒绝!**
